...
void lineqr(){
    ...
    int area_max_i;
    double area_max=0;
    Mat frameToProcess, hsv, mask, X_frame;
...
while (true)
{
    ...
    {
        lock_guard<mutex> lock(frameMutex);
        X_frame = cameraFrame.clone(); // Get a copy of the shared frame
    }
    if(frameToProcess.empty()){
        continue;
    }

    resize(X_frame, frameToProcess ,Size() ,0.5, 0.5);
    //fine black road
...
void opticallflow(){
    ...
    cout<<"right(dx)= "<<x[0]<<"   forward(dy)="<<x[1]<<"     flow quality = "<<flow_quality<<"    fps:"<<fpslive<<endl;
