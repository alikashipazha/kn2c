...
#define THRESH_MISSION_lidarDis 5 //cm
//obstacle
...
int toTheMission_ = 0; // indicates which mission quad's onto
...
void MAVLINK_turnTo(float direction);
bool MAVLINK_isYawInRange();
bool MAVLINK_isAltInRange();
int QR_readData(string &data);
...
int main() {
    ...
    // thread FrontEye(frontEye); //PLAYGROUND

    //joint threads
    if(IntelUnit.joinable()) IntelUnit.join();
    if(BottomEye.joinable()) BottomEye.join();
    // if(FrontEye.joinable()) FrontEye.join(); //PLAYGROUND
...
//QR
int QR_readData(string &data) { // -> QR_decode
    ...
    if(index - toTheMission_ == 1) {
        if(index == 1 && MISSION_DO_1) goOnMission_ = 1;
            ...
            goOnMission_ = 0;
            toTheMission_++;
        } if(DEBUG_TXT && DEBUG_txt_qr) cout << debugID << "toTheMission_ = " << toTheMission_ << endl;
...
void QR_decode(Mat &im, vector<decodedObject>&decodedObjects) { // -> bottomEye
    ...
    for(Image::SymbolIterator symbol = image.symbol_begin(); symbol != image.symbol_end(); ++symbol){
        ...
        dy = ch - meadY; // here
        if(dy < 0 && get_stateMachine() != STATE_QR_turnToDir) return;
...
void bottomEye() {
    ...
    while(true) {
        ...
        if(isOnMissions()) {
            ...
            int subStateMachine = 0, subStateMachineTimer = 0;
            ...
            while(get_stateMachine() == STATE_MISSION_1) {
                ...
                if(subStateMachine == 12) {
                    ...
                    if(MAVLINK_isYawInRange()) subStateMachine++;
                } else if(subStateMachine == 14) {
                    MAVLINK_setAlt(MAVLINK_ALT_normal);
                    if(MAVLINK_isAltInRange()) subStateMachine++;
                } else {
                    set_stateMachine(STATE_DEFAULT);
                    subStateMachine = 0;
                    toTheMission_++;
...
void frontEye() {
    ...
    while(true){
        ...
        if(area_yellow > THRESH_OBS_color){
            ...
            do MAVLINK_setAlt(MAVLINK_ALT_low); while(!MAVLINK_isAltInRange());
            imsg = MSG_DEFAULT;
        ...
        if(area_red > THRESH_OBS_color){
            ...
            do MAVLINK_setAlt(MAVLINK_ALT_high); while(!MAVLINK_isAltInRange());
            imsg = MSG_DEFAULT;
            set_stateMachine(STATE_LINE_underAboveObs);
        }
