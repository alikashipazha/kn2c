...
void set_stateMachine(state_machine state) {
    _stateMachine = state;
    if(DEBUG_TXT) cout << "$ stateMachine = " << _stateMachine << endl;
}
state_machine get_stateMachine() {
    static int timer = 0;
...
void obstacle() {
    ...
    while(true){
        ...
        if(!(get_stateMachine() == STATE_DEFAULT || get_stateMachine() == STATE_QR_getOutOf_box || get_stateMachine() == STATE_OBS_decrease_alt || get_stateMachine() == STATE_OBS_increase_alt)) continue;
        start = clock();
        //resize and hsv
        ...
        if(area_yellow > THRESH_OBS_color){
            ...
            //while alt > THRESH_OBS_alt_yellow decrease alt
        }
        if(area_red > THRESH_OBS_color){
            ...
            //while alt < THRESH_OBS_alt_red increase  alt
        }
...
void lineXqrXland() {
    ...
    while (true) {
        if(!(get_stateMachine() == STATE_DEFAULT || get_stateMachine() == STATE_LINE_getBackInto_rout || get_stateMachine() == STATE_LINE_landing)) continue;
        ...
        if(get_stateMachine() != STATE_LINE_getBackInto_rout && (d[CROP_mid][BLACK] && (d[CROP_firstQuarter][BLACK] || d[CROP_thirdQuarter][BLACK]))) {
...
void decode(Mat &im, vector<decodedObject>&decodedObjects) { // -> lineXqrXland
    ...
    for(Image::SymbolIterator symbol = image.symbol_begin(); symbol != image.symbol_end(); ++symbol){
        ...
        // find center box
        dx = cw - symbol->get_location_x(symbol->get_location_size()-1);
        dy = ch - symbol->get_location_y(symbol->get_location_size()-1);
        while(abs(dx) > THRESH_QR_dx) {
            if(dx < 0) {
                if(DEBUG_TXT && DEBUG_txt_line) cout << debugID << "pitch & roll 15 degree to " << debugRight << endl;
            } else {
                if(DEBUG_TXT && DEBUG_txt_line) cout << debugID << "pitch & roll 15 degree to " << debugLeft << endl;
        ...
        while(abs(dy) > THRESH_QR_dy) cout << debugID << "pitch & roll 15 degree to " << debugForward << endl;
        ...
        switch(readData(obj.data)) {
            ...
            default:
                ...
                isLandingState = true;
                break;
        } decodedObjects.push_back(obj); set_stateMachine(STATE_QR_getOutOf_box);
