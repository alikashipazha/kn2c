// TODO list:
    ...
    //
    // scale line vs land +
    // px4 data : compass(yaw) + alt(z) \ 4th thread?
...
#define SCALE_line_land 0.3 // here
#define SCALE_qr 0.5
...
//alt
#define MAVLINK_ALT_low 0.5
#define MAVLINK_ALT_normal 1
...
//msg
#define MAVLINK_MSG_opf_iteration_delay 3
...
float velocityX, velocityY, desiredYaw, desiredAlt;
...
uint16_t type_mask;
//int16_t mavlink_msg_set_position_target_local_ned_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg, uint32_t time_boot_ms,uint8_t target_system, uint8_t target_component, uint8_t coordinate_frame, uint16_t type_mask, float x, float y, float z, float MAVLINK_VELOCITY_X, float MAVLINK_VELOCITY_Y, float vz, float afx, float afy, float afz, float desiredYaw, float yaw_rate);
...
void obstacle() {
    ...
    while(true){
        ...
        if(area_yellow > THRESH_OBS_color){
            ...
            //while alt > THRESH_OBS_alt_yellow decrease alt ->go to alt //flag decrease //FLAG
                desiredAlt = MAVLINK_ALT_low;
                imsg = MSG_height;
            imsg = MSG_DEFAULT;
        ...
        if(area_red > THRESH_OBS_color){
            ...
            //while alt < THRESH_OBS_alt_red increase  alt -> go to alt ... = define //flag increase //FLAG
                desiredAlt = MAVLINK_ALT_high;
                imsg = MSG_height;
            imsg = MSG_DEFAULT;
            set_stateMachine(STATE_LINE_underAboveObs);
        }
...
void opticalFlow() {
    ...
    while(true){
        switch(imsg) {
            case MSG_velocity: mavlink_msg_set_position_target_local_ned_pack(1, 200, &msg, time_boot_ms, target_system, target_component, coordinate_frame, type_mask, 0, 0, 0, velocityX, velocityY, 0, 0, 0, 0, 0, 0);
            case MSG_yaw :mavlink_msg_set_position_target_local_ned_pack(1, 200, &msg, time_boot_ms, target_system, target_component, coordinate_frame, type_mask, 0, 0, 0, 0, 0, 0, 0, 0, 0, desiredYaw, 0);
            case MSG_height: mavlink_msg_set_position_target_local_ned_pack(1, 200, &msg, time_boot_ms, target_system, target_component, coordinate_frame, type_mask, 0, 0, desiredAlt, 0, 0, 0, 0, 0, 0, 0, 0);                 
            case MSG_land: mavlink_msg_set_position_target_local_ned_pack(1, 200, &msg, time_boot_ms, target_system, target_component, coordinate_frame, type_mask, 0, 0, desiredAlt, 0, 0, 0, 0, 0, 0, 0, 0);
            //case MSG_DEFAULT: mavlink_msg_heartbeat_pack(1, 200, &msg, type, autopilot, base_mode, custom_mode, system_status);
        }
        //here read serialPort
        if(imsg != MSG_DEFAULT) mavlink_msg_to_send_buffer(buf, &msg);
        write(serialPort, &buf, MAVLINK_MAX_PACKET_LEN);
        mavlink_msg_optical_flow_encode(1, 200, &msg, &optical_flow);
        write(serialPort, &buf, MAVLINK_MAX_PACKET_LEN);
        
        if(!(MSG_opf_delay%MAVLINK_MSG_opf_iteration_delay || imsg == MSG_land)) imsg = MSG_DEFAULT;
        MSG_opf_delay++;
...
void lineXqrXland() {
    ...
    stateMachineTimer = 0;
    while (true) {
        ch = (float)(height/2)*SCALE_line_land;
        cw = (float)(width/2)*SCALE_line_land;
        ...
        if(get_stateMachine() == STATE_LINE_goTo_rightAlt) {
            //while myAlt not at THRESH_ALT -> go to THRESH_ALT //flag normal //FLAG
                desiredAlt = MAVLINK_ALT_normal;
                imsg = MSG_height;
        ...
        resize(xframe, frameToProcess ,Size() ,SCALE_line_land, SCALE_line_land);
        //pi camera color map NOT
        ...
        if(get_stateMachine() == STATE_LINE_landing) {
            ...
            if(linesP.size() > 1){    
                ...
                if(abs(dxH) > THRESH_QR_dx) {
                    if(dxH < 0) {
                        if(DEBUG_TXT && DEBUG_txt_line) cout << debugID << "pitch & roll 15 degree to " << debugRight << endl;
                        velocityX = -MAVLINK_VELOCITY_X;
                        velocityY = MAVLINK_VELOCITY_X*MAVLINK_VELOCITY_zigzagRate;
                        //flag
                    } else {
                ...
                } else if(dyH > THRESH_QR_dy) {
                    if(DEBUG_TXT && DEBUG_txt_line) cout << debugID << "pitch & roll 15 degree to " << debugForward << endl;
                    velocityX = 0;
                    velocityY = MAVLINK_VELOCITY_Y/MAVLINK_VELOCITY_Y_forCenterRate;
                    imsg = MSG_velocity;
                    //flag
                } else {
                    if(DEBUG_TXT && DEBUG_txt_land) cout << debugID << "\033[1;44mblue H found with houghline | Stop then decrease altitude\tlargest blue contour area = " << greatestBlueContourArea << "\tdx dy = " << dx << dy << "\033[0m" << endl;
                    desiredAlt = 0;
                    imsg = MSG_land;
                    //flag
                    break;
                ...
                if(!greatestBlueContour.empty()) {
                    ...
                    if(dx < THRESH_LAND_dis_x && dy < THRESH_LAND_dis_y && greatestBlueContourArea > THRESH_LAND_area) {
                        if(DEBUG_TXT && DEBUG_txt_land) cout << debugID << "\033[1;44mblue H found with contour | Stop then decrease altitude\tlargest blue contour area = " << greatestBlueContourArea << "\tdx dy = " << dx << dy << "\033[0m" << endl;
                        desiredAlt = 0;
                        imsg = MSG_land;
                        //flag
                        break;
        ...
        // if(!greatestBlackContour.empty() && !greatestWhiteContour.empty()) {
            ...
            // left_right = (((p0.x - (width/2)*SCALE_line_land))/ ((width/2)*SCALE_line_land))*10;
        ...
        for(int i=0; i < crop_c_len ;i++){
            crop_black = mask_black(Rect(int((width/crop_c_len)*(i)*SCALE_line_land),0,int((width/crop_c_len)*SCALE_line_land),int(height*SCALE_line_land)));
            crop_white = mask_white(Rect(int((width/crop_c_len)*(i)*SCALE_line_land),0,int((width/crop_c_len)*SCALE_line_land),int(height*SCALE_line_land)));
    ...
    for(Image::SymbolIterator symbol = image.symbol_begin(); symbol != image.symbol_end(); ++symbol){
        ...
        if(abs(dx) > THRESH_QR_dx) {
            ...
            switch(readData(obj.data)) { //state get out of box + turn to dir
                case 'N':
                    // if myCompass == compass then set_state getOutOf box //FLAG
                    if(DEBUG_TXT && DEBUG_txt_qr) cout << debugID << "compass 0 until it's 0 -+ 5" << endl;
                    desiredYaw = 0;
                    //flag
                    break;
                case 'E':
                    // if compass then turn ... or if myCompass == compass then set_state getOutOf box //FLAG
                ...
                default:
                    ...
                    isLandingState = true;
                    break;
            } imsg = MSG_yaw; set_stateMachine(STATE_QR_getOutOf_box);
        }
