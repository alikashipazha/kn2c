...
int goOnMission_ = 0; // indicates which mission quad's gonna do after STATE_QR_turnToDir | 0 indicates no mission
int isOnMission_ = 0; // indicates which mission quad's on (single thread)
...
int main() {
    ...
    int input;
    while(toTheMission_ > 4 || toTheMission_ < 1 || isOnMission_ > 4 || isOnMission_ < 1){ //&& (isOnMission_ && toTheMission_ != isOnMission_)){
        cout << "input(10*toTheMission_ + isOnMission_) : ";
        cin >> input;
        toTheMission_ = input/10;
        isOnMission_ = input%10;
    }
    switch(toTheMission_) {
        case 4:
            isLandingState = true;
            break;
        default: break;
    ...
    switch(isOnMission_) {
        ...
        case 3:
            ...
            break;
        case 4:
            set_stateMachine(STATE_LINE_landing);
            break;
    }
    cout << "bkjhkjh" << endl;
    //TEST 
...
char QR_readData(string &data) { // -> QR_decode
    ...
    } if(DEBUG_TXT && DEBUG_txt_qr) cout << debugID << "returning " << data[(toTheMission_-1)*2] << endl;
    return data[(toTheMission_-1)*2];
}
...
bool isOnMissions() {
    switch(isOnMission_) {
        ...
        case 3:
            ...
            return true;
        case 4:
            set_stateMachine(STATE_LINE_landing);//NOTE was STATE_MISSION_4 DO NOT CHANGE
            return true;
    } return false;
}
movement_direction get_movementDirection() {
...
void intelUnit() {
    ...
    while(true){
        ...
        if(!p0.empty()) calcOpticalFlowPyrLK(old_gray, frame_gray, p0, p1, status, err, Size(10,10), 2, criteria);
        // else continue;
        ...
        if(DEBUG_TXT && DEBUG_txt_opf) cout << debugID << "flow_x = " << totalMovementx << " flow_y = " << totalMovementy << " quality = " << flow_quality << endl;
        mavlink_msg_optical_flow_encode(1,200,&msgWrite,&optical_flow_msg);
        mavlink_msg_to_send_buffer(bufWrite, &msgWrite);
